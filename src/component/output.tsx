...
const Output = () => {
  ...
  return (
    <div className='w-full h-full max-md:w-1/2 overflow-hidden p-1'>
      <div className='h-[28px] pl-2 select-none' style={{ color: allTheme.textColor }}><span>Output</span></div>
      <textarea value={lan.Output.output} readOnly className={`w-full h-[calc(100%-33px)] overflow-y outline-none border-2 ${decide.bordercol} resize-none p-2 rounded-md`} style={{ color: allTheme.textColor, fontFamily: codeStyles.fontfamily, background: allTheme.output }}></textarea>
    </div>
  )
}
